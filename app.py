...
# REMOVIDO MAPA DE UNIFICAÇÃO PARA SEPARAR AS CONTAS
# NOME_DISPLAY = { ... } (Desativado)
...
class DatabaseAdapter:
    ...
    def get_all(self):
        try:
            ...
            # AGORA USA O NOME ORIGINAL (SEM MAPA DE UNIFICAÇÃO)
...
def render():
    ...
    df = db.get_all()
    # LISTA DE TODOS OS NICKS (SEM UNIFICAR)
    todos = sorted(list(set(p['nick'].upper() for p in SQUAD_LIST)))
    ...
    if not df_ranking.empty:
        for p in todos:
            ...
            penalidade = 0.0
            if d['D'].mean() <= 2.0 and d['Part'].mean() < 0.35: penalidade -= 25.0
            if d['DPM'].mean() < 300: penalidade -= 10.0
            ...
            ranking_data.append({
                ...
                'DPM': d['DPM'].mean(),
                'Max_DPM': d['DPM'].max(),
                'Total_X1': d['SoloKills'].sum(), # ACUMULADO PARA O CARD
                
                # COLUNAS PARA A TABELA DE AUDITORIA
                'Pts_Win': avg_win, 'Pts_KP': avg_kp, 'Pts_Dano': avg_dpm, 
                'Pts_Torre': avg_obj, 'Pts_Visao': avg_vis, 
                'Total_Accum_X1': d['SoloKills'].sum(), # MUDANÇA AQUI: MOSTRA ACUMULADO
                'Pts_Plates': avg_plates, 'Pts_Multi': avg_multi,
    ...
    with t3:
        if not df_final.empty:
            st.subheader(f"📊 Auditoria Detalhada (Janela de {RANKING_WINDOW})")
            # MUDANÇA: MOSTRAR Total_Accum_X1 NA TABELA AO INVÉS DE PONTOS
            audit_cols = ['Jogador', 'Score Final', 'Pts_Win', 'Pts_KP', 'Pts_Dano', 'Pts_Torre', 'Pts_Visao', 'Total_Accum_X1', 'Pts_Plates', 'Pts_Multi', 'Penalidade']
    ...
    with t4:
        if not df_f.empty:
            ...
            # Join seguro
            comp = comp.join(elo, how='left').fillna("Unranked")
