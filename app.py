...
# Tema CSS inspirado em vkaelin/LeagueStats (Dark & Clean)
st.markdown("""
<style>
    /* Fundo e Fontes */
    ...
    def fetch_flex_matches(self, nome, tag, limit=10):
        try:
            ...
            for i, m_id in enumerate(match_ids):
                bar.progress((i + 1) / len(match_ids), text=f"Analisando partida {i+1}/{len(match_ids)}")
...
def render_dashboard():
    ...
    df = db.get_all()
    if df.empty:
        st.info("👋 Bem-vindo! Comece sincronizando dados na barra lateral.")
        return

    # Tabs separadas para não misturar competitivo e resenha
    ...
    for tab, tipo in [(tab_flex, 'Flex'), (tab_custom, 'Custom')]:
        with tab:
            ...
            # 1. KPIs de Topo (Estilo LeagueStats)
            ...
            # 2. Layout Assimétrico (Tabela Fina | Gráfico Largo)
            col_table, col_graph = st.columns([1, 3])
            
            with col_table:
                st.subheader("Leaderboard")
                # Tabela limpa: Só rank e score total
                ...
                # Detalhes escondidos para limpeza
